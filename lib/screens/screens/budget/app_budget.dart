...
class _BudgetAppState extends State<BudgetApp> with TickerProviderStateMixin {
  ...
  void _addTransaction(Transaction transaction) {
    setState(() {
      ...
      // Update budget spent amount if not excluded from budget
      if (!transaction.excludeFromBudget && transaction.type == TransactionType.expense) {
        final categoryKey = transaction.categoryKey;
        
        // Ensure category exists in budget
        if (!categoryBudgets.containsKey(categoryKey)) {
          categoryBudgets[categoryKey] = {};
        }
        
        // Ensure subcategory exists in budget
  ...
  void _editTransaction(Transaction transaction) {
    showDialog(
      ...
      builder: (context) => AddTransactionDialog(
        ...
        onTransactionAdded: _addTransaction, // Not used in edit mode
  ...
  void _updateTransaction(Transaction updatedTransaction) {
    setState(() {
      ...
      if (index != -1) {
        ...
        // Remove old transaction's budget impact
        ...
        // Add new transaction's budget impact
    ...
    showDialog(
      ...
      builder: (context) => AddTransactionDialog(
        ...
        onTransactionUpdated: (updatedTransaction) {
          // Add the updated transaction to the main transactions list
